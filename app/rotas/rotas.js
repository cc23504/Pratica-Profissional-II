...
module.exports = (app) => {
  ...
  app.get("/", (req, res) => {
    // criando a rota principal que funciona para pagina de apresentação
  ...
  app.post(
    "/interface/AtualizaStatusCarregamento",
    interfaceCon.atualizarStatusCarregamento()
  );
  ...
  app.post("/alugarArmario", (req, res) => {
    ...
    pedidoCon
      .alugarArmario(idCliente, idArmario, tempoUso)
      .then((resultado) => {
      ...
      .catch((erro) => {
        ...
        res
          .status(500)
          .json({ error: "Erro ao alugar o armário", detalhes: erro });
      });
  });
};
